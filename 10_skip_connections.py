...
if __name__ == '__main__':
    ...
    up0 = ConvTranspose1d(in_channels=in_channels * 2, out_channels=out_channels, kernel_size=kernel_size, stride=1)
    up1 = ConvTranspose1d(in_channels=in_channels * 2, out_channels=out_channels, kernel_size=kernel_size, stride=2)
    up2 = ConvTranspose1d(in_channels=in_channels * 2, out_channels=out_channels, kernel_size=kernel_size, stride=2)
    up3 = ConvTranspose1d(in_channels=in_channels * 2, out_channels=out_channels, kernel_size=kernel_size, stride=2)
    ...
    # the minimum we can use is 2^4 = 16
    buffer_size = 16
    ...
    for i in tqdm(range(num_buffers)):
        ...
        z0_i = up0(torch.concatenate([y0_i, z1_i], dim=1))
    ...
    sf.write('output/10_skip_connections_impulse_response.wav', z0 / np.max(np.abs(z0)), 16000)
